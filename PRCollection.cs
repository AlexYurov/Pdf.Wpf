...
namespace Patagames.Pdf.Net.Controls.Wpf
{
	internal class PRCollection : Dictionary<PdfPage, PRItem>
	{
		...
		internal PdfBitmap RenderPage(PdfPage page, Rect actualRect, PageRotate pageRotate, RenderFlags renderFlags)
		{
			if (this.ContainsKey(page))
			{
		...
		/// </summary>
		/// <returns>Null if page still painting, PdfBitmap object if page successfully rendered.</returns>
		private PdfBitmap ProcessExisting(PdfPage page, Rect actualRect, PageRotate pageRotate, RenderFlags renderFlags)
		{
			switch (this[page].status)
			{
				case ProgressiveRenderingStatuses.RenderReader:
					this[page].status =  page.StartProgressiveRender(this[page].bmp, 0, 0, (int)actualRect.Width, (int)actualRect.Height, pageRotate, renderFlags, null);
				...
				default:
					this[page].bmp.FillRectEx(0, 0, (int)actualRect.Width, (int)actualRect.Height, Helpers.ToArgb(Colors.Red));
					this[page].bmp.FillRectEx(5, 5, (int)actualRect.Width-10, (int)actualRect.Height-10, Helpers.ToArgb(Colors.White));
					page.CancelProgressiveRender();
					this[page].status = ProgressiveRenderingStatuses.RenderDone + 2;
		...
		/// </summary>
		private void ProcessNew(PdfPage page, Rect actualRect)
		{
			var item = new PRItem()
			{
				bmp = new PdfBitmap((int)actualRect.Width, (int)actualRect.Height, true),
